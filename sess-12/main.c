...
void spectrogram(char* path, int chosen_channel, int chosen_reduction_power, int frequency_display_reduction_power) {
    /*1. loading of data and FFT */
    waveform_t testform = read_amplitude_data(path, chosen_channel);
    ...
    for(int i = 0; i < nr_formants; i++) {
        ...
        printf("chosen to search between %i and %i;", min_index, max_index);
        ...
        shoud_be_drawn[i] = normalized_frequencies[search_indices[i]] > average;
	...
	while(!WindowShouldClose()) {
		BeginDrawing();
            ...
            DrawSplineLinear(normalized_linear_freuquency_graph, new_len/2, 1.0f, BLUE);
