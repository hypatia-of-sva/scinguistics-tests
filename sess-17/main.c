...
void draw_formant_diagram(formant_diagram_t dia, interval_t* intervals, size_t nr_intervals) {
    pair_t dash;
    ...
    if(intervals != NULL && nr_intervals != 0) {
        for(int i = 0; i < nr_intervals; i++) {
            float xval = dia.base_diagram.base_x + (dia.base_diagram.max_x/dia.base_diagram.nr_of_points)*(intervals[i].upper_index);
            dash[0].x = xval;
            dash[0].y = dia.base_diagram.base_y - dia.base_diagram.max_y - 50.0f;
            dash[1].x = xval;
            dash[1].y = dia.base_diagram.base_y + 50.0f;
            ...
            xval = dia.base_diagram.base_x + (dia.base_diagram.max_x/dia.base_diagram.nr_of_points)*(intervals[i].lower_index);
            dash[0].x = xval;
            dash[0].y = dia.base_diagram.base_y - dia.base_diagram.max_y - 50.0f;
            dash[1].x = xval;
            dash[1].y = dia.base_diagram.base_y + 50.0f;
...
void render(window_state_t state) {
    ...
    draw_formant_diagram(state.log_frequency, NULL, 0);
    draw_formant_diagram(state.linear_frequency, state.intervals, state.nr_intervals);
}
...
window_state_t create_state(waveform_t snippet, int frequency_display_reduction_power, int chosen_reduction_power, char* snippet_name) {
    ...
    interval_t* intervals;
    size_t nr_intervals;
    find_intervals_above_cutoff(normalized_frequencies, len, 0.5, &intervals, &nr_intervals);
    print_intervals(intervals, nr_intervals);

    printf("at 0.05: \n");
    interval_t* intervals2;
    size_t nr_intervals2;
    find_intervals_above_cutoff(normalized_frequencies, len, 0.05, &intervals2, &nr_intervals2);
    print_intervals(intervals2, nr_intervals2);

    printf("merged: \n");
    interval_t* intervals_m;
    size_t nr_intervals_m;
    merge_interval_lists(intervals, nr_intervals, intervals2, nr_intervals2, &intervals_m, &nr_intervals_m);
    print_intervals(intervals_m, nr_intervals_m);

    printf("sorted: \n");
    sort_interval_lists(intervals_m, nr_intervals_m);
    print_intervals(intervals_m, nr_intervals_m);
    */
    ...
    state.nr_intervals = nr_intervals;
...
static int16_t amp_buf[CAPTURE_LEN], tmp_buf[CAPTURE_LEN*2];
    ...
    snprintf(tmp_file_name, 50, "./snippets/%li.wav", time(NULL));

    write_amplitude_data(tmp_file_name, form[0]);
