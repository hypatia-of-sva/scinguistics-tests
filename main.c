...
void spectrogram(char* path, int chosen_channel, int chosen_reduction_power, int chosen_lpc_reduction_power) {
    /*1. loading of data and FFT */
    waveform_t testform = read_amplitude_data(path, chosen_channel);
...
/*
    int lpc_graph_density = 10000;
    float* lpc_function_graph = malloc(lpc_graph_density*sizeof(float));
    float step = (((float)testform.samples_per_second)/((float)lpc_graph_density))/chosen_lpc_reduction_power;

    for(int i = 0; i < lpc_graph_density; i++) {
        lpc_function_graph[i] = lpc_pole_function(formants, step*((float)i));
    }
    float* normalized_lpc = normalize_float_array(lpc_function_graph, lpc_graph_density);

    Vector2* lpc_graph = create_graph_from_float_array(normalized_lpc, lpc_graph_density, 800.0f, 350.0f, 350.0f, 200.0f);
*/
    ...
    matrix_t formants = lpc(testform, 2+2*(testform.samples_per_second/1000));
    print_matrix(formants);
    destroy_matrix(formants);
    
    printf("int-formants \n");
    find_formants(testform, 2+(testform.samples_per_second/1000), NULL, NULL);
    
    printf("float-formants \n");
    matrix_t values, bandwidths;
    size_t nr_formants = 2+(testform.samples_per_second/1000);
    find_formants_2(testform, nr_formants, &values, &bandwidths, true);
    ...
    for(int i = 0; i < nr_formants; i++) {
        ...
        float x_param = log2(formant_in_hz/27.5)/10;
        ...
        if(formant_in_hz != 0.0)
        printf("formant %i at %f Hz; absolute FFT value = %f %%\n", i, formant_in_hz, normalized_frequencies[search_index]*100);
    }
	...
	while(!WindowShouldClose()) {
		BeginDrawing();
            ...
            //DrawSplineLinear(lpc_graph, lpc_graph_density, 1.0f, BLUE);
		EndDrawing();
	}
	CloseWindow();
...
int main(int argc, char** argv) {
    ...
    switch(argv[1][0]) {
        ...
        case 's':
            if(argc < 6) die("not enough arguments!\n");
            spectrogram(argv[2], atoi(argv[3]), atoi(argv[4]), atoi(argv[5]));
        break;
        case 'i':
