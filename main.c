...
void spectrogram_file(char* path, int chosen_channel, int chosen_reduction_power, char* kind, char* new_path) {
    ...
    for(int segment = 0; segment < width; segment++) {
        ...
        for(int y = 0; y < bucket_nr; y++) {
            reduced_bucket_values[y] = pow(heatmap_base[y]/total_frequency, 0.01f);
        ...
        for(int y = 0; y < height; y++) {
            red_numbers[y*width+x] =  get_color_bucket(normalized_bucket_numbers[y]); //ceil(255.0f*normalized_bucket_numbers[y]);
